...
class ManageShiftTablePageState extends ConsumerState<ManageShiftTablePage> {
        ...
        children: [
          ...
          (selectedIndex == 0)
              ...
              : TableEditor(
                  ...
                  onChangeSelect: (p0) async {
                    selectedCoodinate = p0!;
                    if (enableResponseEdit &&
                        shiftTable.requests[selectedIndex - 1].reqTable[p0.row]
                                [p0.column] ==
                            1) {
                      shiftTable.requests[selectedIndex - 1].respTable[p0.row]
                          [p0.column] = requestInputValue;
                      for (int i = 0;
                          i < shiftTable.shiftTable[p0.row][p0.column].length;
                          i++) {
                        if (shiftTable
                                .shiftTable[p0.row][p0.column][i].userIndex ==
                            selectedIndex - 1) {
                          shiftTable.shiftTable[p0.row][p0.column][i].assign =
                              (requestInputValue == 1) ? true : false;
                          break;
                        }
                      }
                      shiftTable.calcFitness(baseDuration.inMinutes,
              ...
              child: Row(
                ...
                children: [
                  ...
                  for (int requesterIndex = 0;
                      requesterIndex < shiftTable.requests.length;
                      requesterIndex++)
                    BottomButton(
                      content: Column(
  ...
  Widget shiftCell(int row, int column, bool editable, bool selected) {
    ...
    var value = shiftTable.shiftFrame.assignTable[row][column] == 0
        ? 0
  ...
  void insertBuffer(List<List<List<Candidate>>> table) {
    undoredoCtrl.insertBuffer(table
        .map((e) => List.from(e
                .map((f) => List.from(f.map((g) => g.copy())).cast<Candidate>())
                .toList())
            .cast<List<Candidate>>())
        .toList());
  }
  ...
  void callUndoRedo(bool undo) {
    setState(() {
      if (undo) {
        shiftTable.shiftTable = undoredoCtrl
            .undo()
            .map((e) => List.from(e
                    .map((f) =>
                        List.from(f.map((g) => g.copy())).cast<Candidate>())
                    .toList())
                .cast<List<Candidate>>())
            .toList();
      } else {
      ...
      shiftTable.calcFitness(
          baseDuration.inMinutes, minDuration.inMinutes, baseConDay);
    });
  }
                    ...
                    if (displayInfoFlag[0])
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            // How to Edit
                            Text("この画面では、「シフトリクエスト期間」終了後、「シフト表」を編集できます。",
                                style: Styles.defaultStyleGrey13),
                            const SizedBox(height: 20),
                            Text("編集方法", style: Styles.defaultStyle18),
                            const SizedBox(height: 10),
                            Text("各日時に対応するマスをタップすると、その日時を希望者の一覧が表示されます。",
                                style: Styles.defaultStyleGrey13),
                            Text(
                                "希望者の名前をタップするとその希望者の「割り当て」/「非割り当て」状態にすることができます。",
                                style: Styles.defaultStyleGrey13),
                            Text("編集後は、画面右上の「登録」ボタンを押して登録してください。",
                                style: Styles.defaultStyleGrey13),
                            Text(
                                "注意 : 編集は常に行うことはできますが、「シフトリクエスト期間」終了後にしか登録できません。",
                                style: Styles.defaultStyleGrey13),
                            const SizedBox(height: 10),
                            Row(
                              mainAxisAlignment: MainAxisAlignment.start,
                            ...
                            const SizedBox(height: 10),
                            Text("シフト表の表示されるアイコンは、その日時の割り当て充足率を示すものです。",
                                style: Styles.defaultStyleGrey13),
...
class RangeFillWidgetState extends State<RangeFillWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    var timeDivs1List = List.generate(
      shiftTable.shiftFrame.timeDivs.length + 1,
      (index) =>
          (index == 0) ? '全て' : shiftTable.shiftFrame.timeDivs[index - 1].name,
    );
    var timeDivs2List = List.generate(
      shiftTable.shiftFrame.timeDivs.length + 1,
      (index) =>
          (index == 0) ? '-' : shiftTable.shiftFrame.timeDivs[index - 1].name,
    );
    var requesterList = List.generate(
      shiftTable.requests.length + 1,
      (index) =>
          (index == 0) ? '全員' : shiftTable.requests[index - 1].displayName,
    );
